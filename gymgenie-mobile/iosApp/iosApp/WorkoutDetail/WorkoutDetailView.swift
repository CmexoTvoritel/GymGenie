...
struct WorkoutDetailView: View {
    ...
    private func exerciseRowView(index: Int, row: ExerciseLineItem) -> some View {
        HStack(spacing: 12) {
            ZStack {
                RoundedRectangle(cornerRadius: 12, style: .continuous).fill(softGray).frame(width: 42, height: 42)
                Image(muscleGroupExerciseImageName(row.muscleGroup))
                    .resizable()
                    .aspectRatio(1, contentMode: .fit)
    ...
    private func editExerciseRow(index: Int, exercise: Shared.PendingExercise) -> some View {
        HStack(spacing: 0) {
            ZStack {
                RoundedRectangle(cornerRadius: 10, style: .continuous).fill(softGray).frame(width: 40, height: 40)
