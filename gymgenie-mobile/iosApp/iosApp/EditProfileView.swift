...
/// Observable form state shared between the edit screen and sub-screens.
/// Mirrors the Android `EditFormHolder` — a single mutable bag of fields
/// seeded once from the loaded profile so sub-screen edits survive
/// navigation.
@MainActor
...
/// A labeled slider that allows both drag and tap-to-edit, matching the
/// Android `SliderField` composable.
...
/// A flow-row chip picker matching the Android `PickerGroup` composable.
...
/// Simple flow layout that wraps children when they exceed the available width.
...
/// Bottom action button shared across edit sub-screens.
