...
/// Modal sheet that lets the user filter exercises by difficulty (multi-select),
/// equipment requirement, and independent sort directions for difficulty and
/// calories.
///
/// State is captured locally so the user can dismiss the sheet without
/// committing changes. The sheet emits a single `onApply` call when the user
/// confirms or resets the selection.
struct ExerciseFilterSheet: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            toggleRow(
                ...
                isOn: Binding(
                    get: { requiresEquipment == true },
                    set: { newValue in
                        requiresEquipment = newValue ? true : nil
                    }
                )
            )
            .padding(.top, 8)
        ...
        .onAppear {
            ...
            // is re-opened with new current* values (the init runs only on
            // first creation of the View struct).
    ...
    /// Two mutually-exclusive sort direction chips (DESC / ASC). Tapping the
    /// active chip clears the selection so the user does not need a separate
    /// "no sort" option. Shared by both sort sections to keep style and
