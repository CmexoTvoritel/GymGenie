...
Single meal type feature reviewed 2026-05-20:
- AiMealFlowStep.CHOOSE(0) added as the new first step. VM default step is now CHOOSE.
- selectMealType(type) sets selectedMealType and immediately advances to PROFILE via goTo(). selectedMealType is nullable; mealType is sent on every chat request (null if user somehow bypasses the CHOOSE step, but CHOOSE is gated to be mandatory).
- mealType nullable in both AiMealChatRequest (KMM) and backend — treated as null in the context message ("не указан"). No hard validation on the backend that mealType must be present. If mealType is null the AI prompt still says "Генерируй план для ОДНОГО приёма пищи" but the "Тип приёма пищи: не указан" context will confuse the model. This is not reachable in normal flow but is an unprotected backend hole.
- Android: CHOOSE step rendered as MealTypeChooseScreen with 3 MealTypeCard composables. Back from CHOOSE calls onDismiss. Correct.
- iOS: AiCoachView (not AiMealCoachView) owns the meal flow navigationDestination chain. AiMealCoachView is dead code (never referenced outside its own file). The CHOOSE screen defined there (AiMealTypeChooseScreen) is defined as `struct` (not `private struct`) and IS referenced from AiCoachView.swift's own chain via AiMealTypeChooseScreen — but that code in AiCoachView.swift (the live path) does NOT exist; the live path goes straight to AiMealProfileScreen. The structs (AiMealTypeChooseScreen, AiMealProfileScreen, AiMealGoalScreen, AiMealRestrictionsScreen, AiMealChatScreen) are defined in AiMealCoachView.swift but used by AiCoachView.swift — the split is intentional and structurally safe. The dead code is the AiMealCoachView struct itself.
- CRITICAL BUG (was): iOS live meal flow (AiCoachView) bypassed the CHOOSE/type-selection step entirely. FIXED in the meal-type-aware conflict detection change: showMealFlow now pushes AiMealTypeChooseScreen, which calls mealVm.selectMealType(type) on selection. iOS and Android are now in parity.

UI fixes batch reviewed 2026-05-21 (settings spacing, profile fallback, filter chip removal, iOS navbar, ExerciseDetailSheetContent reuse):
- COMPILATION BUG: SettingsRow.kt is missing imports for `Spacer` (androidx.compose.foundation.layout.Spacer) and `width` (androidx.compose.foundation.layout.width). The file uses both at lines 57 and 74 but neither appears in the import list. This will fail to compile.
- ExerciseDetailSheetContent: `internal` visibility is correct — both `ExerciseConfigScreen.kt` (feature/create_workout) and `ExerciseInfoSheet.kt` (feature/workout_session/components) are in the same `composeApp` module. No cross-module visibility issue.
- ExerciseConfigScreen.kt: removed local stripped-down sheet, now delegates to shared ExerciseDetailSheetContent. No remaining unused imports — ExerciseDetailViewModel and GlobalContext are used by ExerciseDetailSheetContent itself (in ExerciseInfoSheet.kt), not by ExerciseConfigScreen.kt.
- Profile name fallback (`loaded.firstName ?: loaded.username`): `username` is non-null in UserProfileResponse (line 8, no `?`). Fallback is correct and safe on both Android and iOS.
- Filter chip removal (EquipmentChipsRow): "Все" chip correctly removed from both Android and iOS. Toggle-deselect behavior (tap active chip → null) is identical on both platforms. Correct.
- iOS navbar fix (WorkoutsView.swift): onChange for `catalogExerciseForWorkout` and `showCreateWorkout` now handles both show (non-nil/true → isVisible=false) and hide (nil/false → restore). Logic is correct. Known gap: `showWorkoutSession` has no onChange handler for tabBarState — since it is a fullScreenCover it is not visually broken, but tabBarState stays true while session is showing (same pre-existing pattern). Not introduced by this change.
- SettingsRow spacing: Android uses explicit Spacers (14dp after icon box, 8dp before chevron). iOS uses HStack(spacing: 14) for the overall row + explicit Spacer().frame(width: 8) before chevron when value is present. Not exactly symmetric — Android has 14dp icon-to-label gap (via Spacer) and 8dp value-to-chevron; iOS outer HStack spacing(14) applies between ALL siblings including label-to-value, so there is 14dp between the label and the value text, not just between icon and label. Minor visual inconsistency between platforms in rows with a value. Not a correctness bug.

Create-workout flow improvements (iOS + Android) reviewed 2026-05-21:
- iOS ExercisePickerView: `detailExercise: ExerciseShortResponse?` captures exercise locally before clearing, then calls `onExerciseSelected(captured)`. Safe — no use-after-nil risk.
- iOS ExerciseDetailView: `showAddButton: Bool = true` flag correctly hides button + its safeAreaInset spacer. Propagated correctly to ExerciseConfigView (showAddButton: false) and WorkoutBuilderView's edit sheet (implicitly false via ExerciseEdit path which passes showAddButton: false to ExerciseConfigView's sheet).
- iOS WorkoutBuilderView exercise row spacing: `HStack(spacing: 0)` + `.padding(.leading, 12)` on text column + `Spacer(minLength: 8)` + `HStack(spacing: 8)` for buttons — correct, enforces 8pt inter-button gap and at least 8pt text-to-button clearance.
- iOS CreateWorkoutFlowView `showStepHeader: !startedFromCatalog || stack.count > 1`: CORRECTNESS BUG — when started from catalog, user adds exercise (config→builder), then taps "Добавить упражнение" → stack = [.workoutBuilder, .muscleGroupPicker], then picks muscle group → stack = [.workoutBuilder, .muscleGroupPicker, .exercisePicker], then picks exercise → stack = [.workoutBuilder, .muscleGroupPicker, .exercisePicker, .exerciseConfig(exercise)]. At this point `startedFromCatalog == true` and `stack.count == 4 > 1` → `showStepHeader = true`. This is correct. However the condition `!startedFromCatalog || stack.count > 1` is equivalent to `!startedFromCatalog || stack.count >= 2`, which means when stack.count == 1 AND startedFromCatalog == true → showStepHeader = false. This is exactly the desired initial-entry state. Logic is actually CORRECT for all reachable states.
- Android `showStepHeader = initialExercise == null`: simpler — no stack.count check. When started from catalog, header is ALWAYS hidden on ExerciseConfig step, even when re-entering through the normal flow (after user goes builder → "Добавить упражнение" → group → exercise → config). This means the step header will be missing on subsequent exercises added after the first, when the flow was originally opened from catalog. This is a minor UX inconsistency vs iOS.
- Android `pendingInitialExercise` consumed-once pattern: set before push(WorkoutsConfig.CreateWorkout), consumed in createChild(). If Decompose recreates the child (config change, process death), `pendingInitialExercise` is null → `initialExercise` will be null for the recreated child. The CreateWorkoutFlowScreen will then start from MuscleGroupPicker instead of ExerciseConfig. The `remember { mutableStateListOf(...) }` stack will also restart from MuscleGroupPicker. This is a known Android limitation with transient state — acceptable for the ephemeral create-workout flow.
- ExerciseDetailResponse → ExerciseShortResponse conversion (iOS and Android): `secondsPer10Reps`, `caloriesBurned`, `rating`, `imageUrl` all passed as nil/null. Only `requiresWeight` is critical and IS correctly transferred. The omitted fields are used only for display in ExerciseCard (not needed in create-workout config). Safe.
- Android `category` field omitted in ExerciseShortResponse constructor in WorkoutsContent.kt and CreateWorkoutFlowScreen.kt — uses default `category = ""`. ExerciseShortResponse.category has default = "" — safe.
- iOS WorkoutsView `catalogExerciseForWorkout`: `ExerciseDetailResponse?` state. `fullScreenCover` closure references `detail` from `if let detail = catalogExerciseForWorkout` — safe capture in SwiftUI content closure.
- Back navigation from catalog-initiated flow: iOS — stack.count <= 1 triggers `requestDismiss()` (correct, shows alert if unsaved data). Android — `if (stack.size <= 1) requestDismiss()` (identical behavior). Both correct.

Meal-type-aware conflict detection reviewed 2026-05-21:
- MealPlanEntity gains nullable primaryMealType String? column. ddl-auto:update adds it without migration (acceptable for dev, carries prod schema risk as with all prior nullable additions). No Flyway migration needed for nullable column addition; Hibernate adds it safely.
- MealPlanRepository: two query variants — findScheduledByUserId (no filter) and findScheduledByUserIdAndPrimaryMealType (filters with OR primaryMealType IS NULL to include legacy rows). JPQL OR clause is semantically correct for the stated backward-compat contract.
- findScheduledByUserIdAndPrimaryMealType semantics: legacy null rows are treated as potential conflicts for ANY new meal type. This means a user with an old untyped LUNCH plan will block saving a new BREAKFAST plan — over-conservatively. Documented intent; acceptable backward-compat policy.
- findByUserIdAndMealType query in MealPlanRepository: different from the conflict-check queries. Used by MealPlanService.getBookedDays() to look up scheduling by meal content (JOIN on meals.mealType). The conflict-check path uses primaryMealType on the plan. Two separate query strategies for two different lookups — structurally sound.
- derivePrimaryMealType(): explicit request.mealType takes priority; fallback to first meal's mealType. Both paths validate via isValidMealType(). Returns null if neither is valid. Correct and safe.
- iOS: critical bug fixed — AiCoachView.swift now pushes AiMealTypeChooseScreen on showMealFlow=true; user selects type → mealVm.selectMealType(type) → selectedMealType is set → all subsequent API calls pass the wireValue. Confirmed via code read.
- End-to-end chain: CHOOSE → selectMealType → selectedMealType.wireValue sent on chat, getBookedDays, checkConflicts → doSave passes mealType to SaveMealPlanRequest → backend derivePrimaryMealType stores it. Complete.
- Open edge case (accepted): isValidMealType() hardcodes BREAKFAST/LUNCH/DINNER string comparison. If a new meal type enum value is added, isValidMealType() silently rejects it and falls back to the meals[0] fallback chain, which will also reject the new type → primaryMealType=null. Not a runtime bug now but a forward-compat fragility.
- Open edge case (accepted): null mealType from legacy clients causes findScheduledByUserIdAndPrimaryMealType to be bypassed (falls through to no-filter path). The conflict check then returns any plan regardless of type — safe behavior, just over-conservative for legacy callers.

Meal flow fullScreenCover→navigationDestination migration reviewed 2026-05-20:
- AiMealCoachView.swift: root container struct is now dead code (never instantiated). 4 inner screen structs promoted to internal visibility so AiCoachView.swift can reference them. The root AiMealCoachView struct and its own @StateObject mealVm, onChange handlers, NavigationStack wrapper are all orphaned. No caller references remain. Dead code risk: low in practice, but the file is potentially misleading. Accepted as known.
- AiCoachView: two independent .navigationDestination(isPresented:) chains coexist at the same view level — workout chain anchored on $showProfile (attached to the ZStack body), meal chain anchored on $showMealFlow (also on the ZStack body). Multiple isPresented-based navigationDestination on the same view are legal in SwiftUI iOS 16+ when each state bool is independent. Each chain is self-contained and mutually exclusive by UX (user can only enter one flow at a time from AiChooseScreen). No collision risk.
- Tab bar: onChange(of: showMealFlow) toggles tabBarState.isVisible. showProfile onChange does the same. Since only one can be true at a time, tabBarState correctly hides during either flow. No .onDisappear reset — same pre-existing gap as workout flow (tab-switch-mid-flow leaves tab bar hidden). KNOWN OPEN RISK: user enters either AI flow → switches OS tab → tab bar stays hidden everywhere. Pre-existing, not introduced by this change.
- VM lifecycle: mealVm is @StateObject at AiCoachView level (persists for the tab's lifetime). reset() in onChange(of: showMealFlow) clears KMM state AND sets didPrefill=false. On re-entry, prefillProfile() is called with profileStore.profile before showMealFlow=true. Because prefillProfile guards on didPrefill (which was just cleared by reset()) and on isProfileFilled (KMM state just cleared by reset()), the second run-through correctly re-fills. This is a correct and necessary fix for the lifecycle difference versus the old fullScreenCover approach (which recreated @StateObject on each presentation).
- isSaved/chat state across re-entries: mealVm.reset() calls vm.reset() on KMM side, which resets _state.value = AiMealUiState(). Messages, isSaved, savedPlanId, lastMealPlan are all cleared. User backs out and re-enters to a clean slate. Correct.
- Cascade dismiss (onChange handlers): showMealFlow=false clears showMealGoal/Restrictions/Chat + resets VM. showMealGoal=false clears Restrictions/Chat. showMealRestrictions=false clears Chat. Pattern is identical to workout flow. Correct.
- AiMealCoachView.swift: the now-orphaned AiMealCoachView struct still has its own NavigationStack wrapper, @StateObject mealVm, and profileStore.onChange handlers. These are unreachable but compile. If a future engineer reintroduces AiMealCoachView as a fullScreenCover, it would work standalone — but this intent is not documented. Risk: confusing dead code.
...
Purchase success screen single-CTA + history stats parity reviewed 2026-05-23:
- PurchaseSuccessScreen (Android) and PurchaseSuccessView (iOS): two buttons ("Начать с планом от ИИ" + "Продолжить без плана") replaced with single GymGenieButton "Освоить новые возможности". Image retained on both. Subtitle text updated to "Теперь вам доступны все возможности приложения для достижения ваших целей".
- Both platforms navigate to home on tap (iOS: appState.completeOnboardingFlow() → .main; Android: onPurchaseSuccessContinue() → navigation.replaceAll(Main) AND closePaywall() path). Correct.
- HistoryCard.kt (Android) and WorkoutHistoryView.swift (iOS): for incomplete workouts now show two text lines ("Упражнения: X из Y" + "Подходы: X из Y") with guard on totalExercises > 0 and totalSets > 0. Matches iOS behavior. completedExercises field confirmed in KMM WorkoutSessionHistoryItem and backend DTO. Correct.
- Both platforms guard each line independently (if totalExercises > 0 / if totalSets > 0). Edge case: when BOTH are 0 (e.g. session abandoned immediately), the incomplete branch renders nothing at all below the progress bar. Same behavior on both platforms — acceptable.
- GymGenieButton parameter name: iOS uses `title:`, Android uses `text:` — different component signatures, platform-specific, correct.
- VStack wrapper around single button on iOS (VStack(spacing:12){...}) is a leftover container from the two-button era. No visual impact but slightly redundant. Non-blocking.

Purchase success screen bug fix reviewed 2026-05-22:
- Android: MainContent.kt now shows PurchaseSuccessScreen in-place when showingPurchaseSuccess=true, PaywallScreen otherwise — both inside the isPaywallActive guard. Correct fix.
- KNOWN OPEN RISK (Android): showingPurchaseSuccess is declared with remember{} in MainContent, which means it resets to false on Activity recreation (config change). If the user buys, reorients the device before tapping Continue, the success screen will be gone and the paywall will reappear. The PaywallScreen's purchaseSuccess state in the KMM VM will also be reset (remember creates a new PaywallViewModel via Koin factory). Net result: success screen lost, user sees the paywall again. Low-probability since the success screen is meant to be tapped quickly, but technically losable.
- KNOWN OPEN RISK (Android): showingPurchaseSuccess does NOT reset when the paywall is reopened. If a user: buys → sees success screen → taps Continue → closes paywall (state resets to false and closePaywall() fires) → reopens paywall → normal PaywallScreen is shown (correct). This path is fine. But if somehow closePaywall() is called WITHOUT the user tapping Continue (can't happen in current code since the only way to close paywall is via onSkip or onContinue), state leakage could show success screen on a fresh paywall open. Not a reachable bug currently.
- iOS: .task(id: viewModel.purchaseSuccess) fires when PaywallView appears (id is initially false → task fires but guard `if viewModel.purchaseSuccess` blocks execution). When purchase completes, purchaseSuccess becomes true → task fires → consumePurchaseSuccess() + appState.completePurchase() → navigate(.purchaseSuccess). Correct.
- iOS IMPORTANT: the fix only applies to the onboarding paywall (AppState.currentScreen == .paywall). The in-app paywall from HomeView and AiCoachView is triggered via appState.navigate(to: .paywall), which replaces MainView with PaywallView in ContentView. This means the same PaywallView and same .task(id:) fix covers both the onboarding and in-app cases. There is no separate paywall sheet inside MainView — the navigate(.paywall) call tears down MainView and shows the top-level PaywallView.
- iOS task(id:) initial-fire concern: task fires once on appear with id=false. The `if viewModel.purchaseSuccess` guard prevents any false positive. When purchaseSuccess flips true→false (after consumePurchaseSuccess()), the task fires again with id=false — same guard blocks it. Clean.

Exercise emoji → category image migration reviewed and COMPLETED 2026-05-22:
- Android: muscleGroupExerciseDrawable() added in MuscleGroupEmoji.kt (8 mapped images, BICEPS/TRICEPS/FOREARMS→arms, QUADRICEPS/HAMSTRINGS/GLUTES/CALVES→legs). All screens updated including ExerciseHero.kt and RestTimerScreen.kt. muscleGroupDrawable() (picker) untouched. Existing ic_chest/ic_back/etc. drawables intact.
- iOS: muscleGroupExerciseImageName() added in MuscleGroupEmoji.swift (identical mapping). All listed iOS screens updated. ic_exercise_* imagesets added to Assets.xcassets. Existing picker imagesets intact.
- ExerciseHero.kt: muscleGroup: String? parameter added; call site in WorkoutSessionScreen.kt passes exercise?.muscleGroupLabel. Correct.
- RestTimerScreen.kt: nextMuscleGroup() private helper added; next-exercise card renders muscleGroupExerciseDrawable(nextMuscleGroup(state)). Correct.
- iOS RestTimerView.swift: uses Image(systemName: "dumbbell") in nextExerciseCard — generic SF Symbol. This is an INTENTIONAL DIVERGENCE from Android (which shows category image). Accepted.
- MuscleGroupPickerScreen (Android) and MuscleGroupPickerView (iOS): correctly use muscleGroupDrawable() / muscleGroupImageName() respectively — NOT exercise images. Correct.
- Dead code (known, harmless): muscleGroupEmoji() function definitions remain in ExerciseCard.swift, WorkoutHistoryView.swift, ExerciseDetailView.swift, and MuscleGroupEmoji.swift — never called. No live rendering path uses emojis for exercises anymore.
- ContentScale.Crop vs .Fit inconsistency: ExerciseCard and ExerciseConfigScreen use Fit+aspectRatio(1f); WorkoutBuilderScreen, WorkoutPlanCard, WorkoutDetailScreen, HistoryCard use Crop in square containers. Functionally identical for 1:1 source images.

Long-press delete activity + centering fix reviewed 2026-05-23:
- ActivitiesViewModel.removeFromPlan(): optimistic delete (filter out), rollback via `current.todayActivities + removed` on failure. COSMETIC ISSUE: rollback always appends removed to end of list, not to original position. If user deletes the 2nd of 5 items and backend fails, the item reappears at position 5.
- HomeViewModel.removeFromPlan(): same pattern, same ordering-on-rollback issue. Has `activityError` field and shows it via snackbar on Android — correct user feedback. ActivitiesViewModel has NO error field for removeFromPlan failure — rollback is silent on iOS and Android ActivitiesScreen (no snackbar/toast).
- iOS HomeView.swift: onRemoveFromPlan routes to `activitiesViewModel.removeFromPlan()` NOT `viewModel.removeFromPlan()`. HomeView observes `viewModel.todayActivities` (HomeViewModel), not ActivitiesViewModel. So the optimistic removal fires on a VM whose state is not observed by the Home screen. Item does NOT disappear immediately. `viewModel.refresh()` is called instead — a full network round-trip. On Android the HomeScreen correctly calls `viewModel::removeFromPlan` (HomeViewModel). Platform inconsistency: broken optimistic-update on iOS Home screen only.
- ActivitiesScreen centering fix: `Modifier.fillMaxSize()` on both LazyColumns inside HorizontalPager — correct fix. LazyColumn defaults to wrapContentSize; HorizontalPager provides maxHeight constraint causing centering. fillMaxSize() forces full page height. Standard correct approach.
- deleteTarget uses `remember` (not `rememberSaveable`) — correct, sheet should not survive config change.

Activity icon badge migration (emoji → ring icon) reviewed 2026-05-23:
- Android: ActivityCard, CatalogActivityCard, PresetBottomSheet, ActivityCatalogScreen all correctly use painterResource(R.drawable.ic_activity_run/mind/schedule) with no ColorFilter. Image import is androidx.compose.foundation.Image (consistent with project).
- DUPLICATE ringColorFor: CatalogActivityCard.kt defines its own internal ringColorFor() (fallback → AccentOrange). ActivityCard.kt also defines ringColorFor() (fallback → DeepInk). ActivitiesScreen imports ActivityCard's version. The two functions have different fallback colors. ActivityCatalogScreen imports CatalogActivityCard's version. This is a pre-existing dual-definition, not introduced by this change.
- iOS EmojiBadge (HomeView.swift): `name: String` parameter exists on the struct but is never read in body — unused stored property. Caller passes `activity.name` into it. Dead field but compiles and does not affect behavior.
- iOS HistoryActivityRow binary completion label: shows "Done" (English) instead of Russian-language localized string. All surrounding labels in the UI are Russian. Inconsistency.
- Icon sizing: Android ActivityCard 42dp container/22dp icon; PresetBottomSheet 48dp/24dp; CatalogActivityCard 36dp/18dp. iOS EmojiBadge 42pt/22pt; PresetSheetView 48pt/24pt; CatalogActivityCard iOS 36pt/18pt. Icon sizes proportionally consistent at 50-52% of container across all sites. No tint applied on either platform.
- No ColorFilter/renderingMode applied anywhere on the new icons — correct.

Edit-flow wrong icon fix + iOS bottom sheet image fix reviewed 2026-05-23:
- ROOT CAUSE FIX: CreateMealPlanViewModel.loadForEditing() now maps dish.foodCategory via FoodCategory.fromKeyOrOther() with fallback to OTHER. Previously hardcoded FoodCategory.OTHER on every FoodProduct reconstruction. Fix is correct and complete.
- fromKeyOrOther() confirmed: uses entries.firstOrNull { it.name == raw } ?: OTHER — exhaustive over all 11 enum values, safe for unknown future backend values.
- loadForEditing() is the ONLY place in the codebase that constructs FoodProduct from MealPlanDetailDish. ProductCard.kt (Android) and ProductCardView.swift (iOS) in the detail view read dish.foodCategory directly without going through FoodProduct — not affected.
- iOS GramsSheetContent: changed from ZStack+background+30dp image to Image(foodCategoryImageName(product.category)).resizable().aspectRatio(1,.fill).frame(48,48).clipShape(RoundedRectangle(14)). product.category is FoodCategory (domain type), matches foodCategoryImageName signature — no wire-string remapping needed here. Correct.
- iOS EditGramsSheetContent: identical treatment. Correct.
- Android GramsSheet: already uses categoryBg+fillMaxSize+aspectRatio(1f)+ContentScale.Crop — was already correct, no change needed.
- No other screen reads MealPlanDetailDish and reconstructs a FoodProduct with a hardcoded category — no additional bug sites exist.

Food/meal plan UI fixes reviewed 2026-05-23 (foodCategory propagation + image/sizing fixes):
- COMPILATION BUG: MealPlanService.kt uses `FoodCategory` type in the local `ScaledMacros` data class (line 129) but `FoodCategory` is NOT imported. The import list includes DishEntity, FoodProductEntity, MealEntity, MealPlanEntity, MealType, NutritionCreatedBy — but NOT FoodCategory. This will fail to compile. Fix: add `import com.asc.gymgenie.nutrition.entity.FoodCategory`.
- DishEntity.kt: `foodCategory: FoodCategory?` field with `@Enumerated(EnumType.STRING)` and `@Column(name = "food_category", length = 32)` — correct JPA mapping.
- DishResponse DTO: `foodCategory: String? = null` — correct, enum name as string. mapper uses `foodCategory?.name` — correct.
- MealAiService.kt: `foodCategory = product?.category` — correct, product is nullable (AI dishes may not have a catalog match).
- MealPlanService.kt: for catalog products, `foodCategory = product.category`; for AI inline dishes, `foodCategory = null` — correct two-path handling.
- Migration SQL: `ALTER TABLE dishes ADD COLUMN IF NOT EXISTS food_category VARCHAR(32)` — correct idempotent backfill alongside full CREATE TABLE definition. Consistent with ddl-auto:update project pattern.
- KMM MealPlanDetailDish: `foodCategory: String? = null` added — correct, nullable with default for backward compat.
- iOS FoodCategoryImage.swift: `foodCategoryFromWireValue()` maps backend enum strings (MEAT/FISH/DAIRY/EGGS/GRAINS/LEGUMES/VEGETABLES/FRUITS/NUTS_SEEDS/OILS/OTHER) to FoodCategory — exhaustive and correct.
- ProductCardView.swift: now uses `foodCategoryFromWireValue(dish.foodCategory)` → `foodCategoryImageName()` with fallback to `deriveFoodImageName()`. Compact 48x48 HStack layout. Dead code: `deriveEmoji()` function still present, never called.
- Android ProductCard.kt: uses `FoodCategory.valueOf(categoryStr).categoryDrawable()` with try-catch fallback to `deriveFoodDrawable(dish.name)` — safe enum parsing.
- iOS CreateMealPlanFlowView.swift: ProductRow image fills 48x48 via `aspectRatio(1, contentMode: .fill)` + `clipShape(RoundedRectangle)`. AddedItemRow fills 44x44 similarly. Calories icon 44x44 + `.renderingMode(.template)` + `.foregroundColor(.red)`. Edit/delete buttons 36x36, icon font 16.
- Android CreateMealPlanFlowScreen.kt: InfoStep uses `aspectRatio(1f)` + `fillMaxSize()`. Calories icon `size(44.dp)` + `ColorFilter.tint(Color.Red)`. Edit/delete boxes `size(36.dp)`, icons 18dp.
- iOS InfoStep: keeps ZStack with `categoryBg()` + full-area image — intentional (colored background behind the food image). Android equivalent uses `categoryBg()` color + `fillMaxSize()` image. Both correct for their patterns.
- KNOWN OPEN: Dead emoji functions (`defaultEmoji()` in CreateMealPlanFlowView.swift, `deriveEmoji()` in ProductCardView.swift, `deriveEmoji()` in ProductCard.kt) remain. Harmless but clutters files. Pre-existing, carried forward.
- MealPlanSection.kt and HomeView.swift not touched — meal-type emojis correctly untouched.
- iOS ProductRow in PickerStep: image fills 48x48 with `clipShape(RoundedRectangle)` — ZStack background removed, now clean. Consistent with Android.
- iOS InfoStep image area: uses ZStack with categoryBg + full image fill — same visual result as Android's background+fillMaxSize pattern.

username→firstName full rename fix reviewed 2026-05-23 (second pass, replaces the partial name-field fix):
- Root cause (duplicate display name uniqueness constraint) fully resolved: `unique=true` removed from `UserEntity.username` `@Column`; `existsByUsername()` and `findByUsername()` removed from UserRepository; `existsByUsername()` check removed from AuthService.register(). Username field is now nullable String? = null with no constraint.
- Backend DTOs: RegisterRequest.firstName, AuthUserResponse.firstName (mapped from entity.firstName ?: ""), UserProfileResponse previously had `name` removed — now only firstName/lastName exist.
- UserEntity: username kept as nullable remnant column (String? = null), firstName populated at registration. generateTokenResponse maps firstName ?: "" — safe for new users; existing users with null firstName will get empty string in token response.
- KMM AuthModels: RegisterRequest.firstName, UserResponse.firstName (non-null String). AuthApi.register(firstName:...) — correct. UserProfileResponse.name removed, only firstName/lastName remain.
- AuthViewModel.register(): passes `current.name.trim()` as `firstName` parameter — AuthUiState still uses `name` field internally as the UI-layer text field binding. This is a VIEW-STATE field name, NOT the removed DTO field. Correct.
- HomeViewModel.fetchProfile(): sets `name = profile.firstName ?: ""` in HomeUiState.name. HomeUiState.name is a view-layer string, not a DTO field. Correct.
- iOS wrappers: AuthViewModelWrapper.name and HomeViewModelWrapper.name map from the KMM view-state `name` field — NOT from the removed UserProfileResponse.name. Safe.
- iOS buildDisplayName() in ProfileView: `[p.firstName, p.lastName].compactMap { $0 }.filter { !$0.isEmpty }`. If both null, returns "" (empty string, not "друг"). Accepts the UX gap — profile page shows blank name for users with null firstName.
- iOS buildHomeDisplayName() in HomeView: `p.firstName` → "друг" fallback. No reference to removed name field. Correct.
- Android ProfileScreen displayName: buildString with firstName/lastName, redundant second line `if (isEmpty()) profile?.firstName?.let { append(it) }` — dead code harmless.
- Android HomeScreen buildDisplayName(): firstName → "друг" fallback. Correct.
- ddl-auto:update note: existing DB with unique constraint on username column will NOT have it dropped automatically. PostgreSQL treats multiple NULLs as distinct in unique indexes, so all new users with username=null are safe. No migration needed for correctness, but the stale unique index remains on the DB.
- SQL file `drop_username_unique_constraint.sql` DELETED (no longer needed since the constraint removal is handled by the column annotation change and the null-safe PostgreSQL behavior).
- No references to old `UserProfileResponse.name` or `UserResponse.name` remain anywhere in the live codebase (confirmed by grep).

Food category emoji → ic_food_* image migration reviewed 2026-05-22:
- Android: FoodCategoryImage.kt added in feature/food_picker (FoodCategory.categoryDrawable() + deriveFoodDrawable()). categoryDrawable() is a when-expression with all 11 FoodCategory values covered — exhaustive, no else needed.
- iOS: FoodCategoryImage.swift added in MealPlanDetail/ folder (foodCategoryImageName() + deriveFoodImageName()). Uses `import Shared` — required because FoodCategory is a KMM shared type. The import IS correct and necessary despite the task spec saying it "shouldn't need it."
- 11 ic_food_*.imageset directories added to iOS Assets.xcassets. Contents.json only has 1x scale with the PNG; 2x and 3x entries are empty. This means on 2x/3x devices iOS will upscale the 1x asset, matching Android which also only has base drawable/ (no hdpi/xhdpi/etc.). Neither platform has hi-res variants. Consistent but both platforms display scaled-up images on Retina/HDPI screens.
- Category mapping consistent between platforms: MEAT→meat, FISH→fish, DAIRY→milk, EGGS→egg, GRAINS→cereals, LEGUMES→legumes, VEGETABLES→vegetables, FRUITS→fruits, NUTS_SEEDS→nuts, OILS→oil, OTHER→other. Verified.
- deriveFoodDrawable (Android) and deriveFoodImageName (iOS): keyword lists are identical (same Russian stems, same ordering). LEGUMES keywords ("фасол", "горох", "чечевиц", "нут", "бобов") appear AFTER OILS in iOS but AFTER NUTS in Android — ordering difference but because these are non-overlapping keyword sets, it produces identical results for all inputs. Not a bug.
- Dead code left in all 5 files: defaultEmoji()/deriveEmoji() functions defined but never called in FoodPickerScreen.kt, CreateMealPlanFlowScreen.kt, ProductCard.kt (Android) and CreateMealPlanFlowView.swift, ProductCardView.swift (iOS). These are the old emoji functions superseded by the new image functions. Never called — purely dead code.
- Meal type emojis (☀️, 🥗, 🌙, 🍎 for SNACK, 🍽️ for else) in MealPlanSection.kt, ScheduleChip.kt, HomeView.swift correctly untouched — these are meal-type palette emojis, not food category icons. Correct.
- KNOWN OPEN: The old emoji functions in FoodPickerScreen.kt (defaultEmoji), CreateMealPlanFlowScreen.kt (defaultEmoji), ProductCard.kt (deriveEmoji), CreateMealPlanFlowView.swift (defaultEmoji), and ProductCardView.swift (deriveEmoji) are dead code but were not removed. Low priority but clutters the files.

iOS 26 Liquid Glass TabView added 2026-05-20:
- MainView.body branches on #available(iOS 26, *): liquidGlassTabView (native TabView) vs legacyTabView (custom ZStack + BottomNavBar).
- Tab struct (init(_:systemImage:value:content:)) is available from iOS 18, not iOS 26. The #available(iOS 26, *) guard is intentional and correct for Liquid Glass styling specifically, but the Tab API itself is not limited to 26.
- .toolbar(tabBarState.isVisible ? .visible : .hidden, for: .tabBar) is applied on the TabView container (line 90), NOT on child views. SwiftUI .toolbar visibility for .tabBar is best applied inside the content of NavigationStack descendants, not on the TabView itself. This creates a critical architectural risk: child views (AiCoachView, HomeView, WorkoutsView) write tabBarState.isVisible to drive legacy hide/show behavior; in iOS 26 path that state change triggers recomposition of MainView which re-evaluates the .toolbar modifier on the TabView. Whether this reliably hides/shows the Liquid Glass tab bar at the TabView level is unverified — SwiftUI documentation recommends applying this modifier to leaf views inside the tab, not the TabView itself.
- environmentObjects: profileStore and tabBarState are correctly passed in both paths. AppState is NOT passed directly by MainView (it comes from ContentView one level up via .environmentObject in ContentView.body — this is correct and unchanged).
- onAppear { profileStore.load() } is present in both paths.
- onChange closure forms: iOS 26 path uses two-parameter { _, tab in } (iOS 17+ API); legacy path uses single-parameter { tab in } (iOS 14+ API). Both are correct for their respective guard conditions since the legacy path has no @available restriction.
- navItems computed property (for BottomNavBar) is defined at struct level but only used in legacyTabView — no waste, just dead in iOS 26 path.
- KNOWN OPEN RISK (2026-05-20): .toolbar visibility applied at TabView level in iOS 26 path may not propagate correctly to Liquid Glass tab bar. Standard pattern requires applying inside the NavigationStack content. If tab bar doesn't hide on iOS 26 when AiCoachView enters a flow, the UX will be broken.
